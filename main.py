...
def hide_status_bar():
    ...
    if platform == 'android':
        ...
        # 1. Разрешаем рисовать фон приложения под системными панелями
            window.addFlags(WindowManager.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS)
        
        # 2. Делаем обе панели прозрачными
            window.setStatusBarColor(Color.TRANSPARENT)
            window.setNavigationBarColor(Color.TRANSPARENT) # Делает низ прозрачным

        # 3. Устанавливаем флаги, чтобы контент "затекал" под кнопки навигации
            decorView = window.getDecorView()
            decorView.setSystemUiVisibility(
            View.SYSTEM_UI_FLAG_LAYOUT_STABLE | 
            View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN |
            View.SYSTEM_UI_FLAG_LAYOUT_HIDE_NAVIGATION # Ключевой флаг для нижней панели
        )
