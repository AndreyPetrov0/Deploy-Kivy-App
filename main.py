...
def hide_status_bar():
    ...
    if platform == 'android':
        ...
        # 1. Снимаем все ограничения по размеру окна
            Window.addFlags(LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS)
            Window.clearFlags(LayoutParams.FLAG_TRANSLUCENT_STATUS)
        
        # 2. Устанавливаем прозрачность на уровне Window
            Window.setStatusBarColor(Color.TRANSPARENT)
            Window.setNavigationBarColor(Color.TRANSPARENT)

        # 3. Принудительно растягиваем View под панели через флаги видимости
        # Это самый важный блок для того, чтобы убрать черную полосу
...
class Level_SpotterApp(App):
    ...
    def enable_android_bars(self):
        ...
        def show_bars():
            ...
            # Очищаем флаг Fullscreen, чтобы показать статус-бар
            window.clearFlags(WindowManager.FLAG_FULLSCREEN)
            
            # Сбрасываем системные флаги видимости для показа панели навигации
